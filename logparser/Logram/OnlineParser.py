...
def OnlineParser(log_format, logFile, rex, doubleThreshold, triThreshold, outAddress):
    ...
    templateTable = {}
    outFile = codecs.open(outAddress + "eventOL.txt", "w", encoding='utf-8', errors='ignore')
    templateFile = codecs.open(outAddress + "template.csv", "w", encoding='utf-8', errors='ignore')

    regex = regexGenerator(log_format)
    for line in codecs.open(logFile, 'r', encoding='utf-8', errors='ignore').readlines():
        #print(line)
