...
if __name__ == '__main__':
    ...
    for dataset, setting in benchmark_settings.items():
        ...
        log_messages = TokenizeGroupLayer(log_messages, rex=[]).run()
        ...
        with open("PT_SwissLog.txt", "a") as f:
               f.write(os.path.basename(log_file).split('.')[0]+' '+str(endtime - starttime)+' ' + str(mem.total) + ' ' + str(mem.used) + ' ' + str(mem.available) + ' ' + str(mem.percent) + '\n')
