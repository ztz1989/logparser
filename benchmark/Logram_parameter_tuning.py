...
for dataset, setting in benchmark_settings.iteritems():
    ...
    doubleThreshold = np.arange(10, 500, 10)
    triThreshold = np.arange(10, 500, 10)

    for d in doubleThreshold:
	for t in triThreshold:
		print("Double Threshold, Triple Threshold: ", d, t)
		start_time = datetime.now()
    		doubleDictionaryList, triDictionaryList, allTokenList = dictionaryBuilder(setting["log_format"], log_file, [])
    		tokenMatch(allTokenList,doubleDictionaryList,triDictionaryList,d,t,'Logram_result/'+dataset)
    		...
    		benchmark_result.append([dataset, d, t, f_measure, accuracy])

    print('\n=== Overall evaluation results ===')
    df_result = pd.DataFrame(benchmark_result, columns=['Dataset', 'doubleThreshold', 'triThreshold', 'F1_measure', 'Accuracy'])
    df_result.set_index('Dataset', inplace=True)
    print(df_result)
    df_result.to_csv('Logram_benchmark_result_' + dataset + '.csv')
